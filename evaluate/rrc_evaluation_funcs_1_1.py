...
#Date: 2019-12-29
# Description: File with useful functions to use by the evaluation scripts
# in the RRC website.
...
def get_tl_line_values(
    ...
    if LTRB:
        ...
        if(ymax < ymin):
            raise Exception(
                "Ymax value (%s)  not valid (Ymax < Ymin)." %
                (ymax))
        ...
        if (imWidth > 0 and imHeight > 0):
            ...
            validate_point_inside_bounds(
                points[2], points[3], imWidth, imHeight)
            validate_point_inside_bounds(
                points[4], points[5], imWidth, imHeight)
            validate_point_inside_bounds(
                points[6], points[7], imWidth, imHeight)
...
def get_tl_dict_values(
    ...
    for i in range(num_points):
        if isinstance(detection['points'][i], list) == False:
            raise Exception("Incorrect format. Point #" +
                            str(i + 1) + " has to be an array)")

        if len(detection['points'][i]) != 2:
            raise Exception("Incorrect format. Point #" + str(i + 1) +
                            " has to be an array with 2 objects(x,y) )")

        if isinstance(
                detection['points'][i][0], (int, float)) == False or isinstance(
                detection['points'][i][1], (int, float)) == False:
            raise Exception("Incorrect format. Point #" +
                            str(i + 1) + " children have to be Integers)")

        if (imWidth > 0 and imHeight > 0):
            validate_point_inside_bounds(
                detection['points'][i][0],
                detection['points'][i][1],
                imWidth,
                imHeight)
    ...
    if withTranscription:
        ...
        # Ensures that if illegibility attribute is present and is True the
        # transcription is set to ### (don't care)
        if 'illegibility' in detection:
            if detection['illegibility']:
                transcription = "###"

        # Ensures that if dontCare attribute is present and is True the
        # transcription is set to ### (don't care)
